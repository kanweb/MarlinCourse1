...
<main id="js-page-content" role="main" class="page-content mt-3">
    ...
    <div class="row" id="js-contacts">
        ...
        if (!$users && empty($users)) {
            set_flash_message('alert', '<strong>Уведомление!</strong><br> Произошла непредвиденная ошибка, попробуйте зайти позднее');
            redirect_to("page_login.php");
        }
        foreach ($users as $key => $user) { ?>
            <? //print_r($user); ?>
            <div class="col-xl-4">
                <div id="c_<?= $key + 1 ?>" class="card border shadow-0 mb-g shadow-sm-hover"
                     data-filter-tags="<?= $user['username'] ?>">
                    <div class="card-body border-faded border-top-0 border-left-0 border-right-0 rounded-top">
                        <div class="d-flex flex-row align-items-center">
                                <a href="page_profile.php?id=<?=$user['id']?>"><span class="status status-success mr-3" >
                                    <span class="rounded-circle profile-image d-block "
                                          style="background-image:url('<?= ($user["avatar"]) ?: "/img/demo/avatars/avatar-m.png" ?>'); background-size: cover;"></span>
                                    </span></a>
                            <div class="info-card-text flex-1">
                                <a href="javascript:void(0);" class="fs-xl text-truncate text-truncate-lg text-info"
                                   data-toggle="dropdown" aria-expanded="false">
                                    <?= $user['username'] ?>
                                    <? if (is_admin() || $_SESSION['user']['id'] == $user['id']) { ?>
                                ...
                                <? if (is_admin() || $_SESSION['user']['id'] == $user['id']) { ?>
                                    <div class="dropdown-menu">
                                        ...
                                        <a href="delete.php?id=<?= $user['id'] ?>" class="dropdown-item" onclick="return confirm('Вы уверены?');">
